...
@pre
*/
void insertarDatos(char v_cadena[],int &util_v_cadena,const int DIM_V_CADENA){


		//Pedimos que intrduzca el numero de letras que quiere introducir.

		do{

			cout << "Introduzca el número de letras que quieres introducir (entre 1 y )" << DIM_V_CADENA << "): ";
			cin >> util_v_cadena;

		}while(util_v_cadena<1 || util_v_cadena>DIM_V_CADENA);

		//Le pedimos tantos números en funcion a la variable util.

		for(int i=0;i<util_v_cadena;i++){
...
int main(){
			...
			insertarDatos(v_cadena,util_v_cadena,DIM_V_CADENA);
